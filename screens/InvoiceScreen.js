...
export default function InvoiceScreen() {
  ...
  // Order Details
  const customerName = booking?.fromAddress?.receiverName || booking?.userName || "Customer Name";
  const vehicleType = booking?.vehicleType || "Vehicle Details";
  const driverName = booking?.riderName || booking?.driverName || "Driver Name";
  const natureOfGoods = booking?.goodsType || booking?.itemDescription || "Nature Of Goods";
  const quantity = booking?.quantity || "1";

  // Payment Details
  const tripFare = booking?.priceBreakdown?.components?.baseFare || booking?.fare || booking?.price || 0;
  const couponDiscount = booking?.discount || booking?.couponDiscount || 0;
  const roundingOff = booking?.roundingOff || 0;
  const tollCharges = booking?.tollCharges || booking?.priceBreakdown?.components?.tollCharge || 0;
  const totalFare = booking?.totalAmount || booking?.amountPay || booking?.price || 0;

  // Address Details
  const pickupLocation = booking?.fromAddress?.address || booking?.pickupAddress || "Pickup Location";
  const droppingLocation = booking?.dropLocation?.[0]?.Address || booking?.dropAddress || "Dropping Location";

  // Parcel Photo
  const parcelPhoto = booking?.parcelImage || booking?.deliveryImages?.[0] || booking?.bookingImages?.[0];

  // Footer Details
  const gstCategory = "Regular";
